...
function toggleCategoryNatrium(categoryId) {
    ...
    if (content && header) {
        ...
        ['pokok', 'masakan', 'lauk', 'sayur', 'camilan'].forEach(c => {
...
function searchFoodNatrium(query) {
    ...
    ['pokok', 'masakan', 'lauk', 'sayur', 'camilan'].forEach(cat => {
                ...
                <div class="p-3 rounded-4 insight-box bg-primary bg-opacity-10 border border-primary border-opacity-25">
                    <div class="d-flex gap-2 align-items-start">
                        ...
                        <div class="small text-muted">
                            ...
                            Menu <strong>${maxFood}</strong> adalah penyumbang natrium terbesar yaitu <strong>${maxNa}mg</strong>. 
...
const NATRIUM_DATABASE = {
    makanan_pokok: {
        ...
        kentang_rebus: { nama: "Kentang Rebus", natrium: 7, urt: "1 buah sdg" },
    ...
    masakan: {
        ...
        gudeg_jogja: { nama: "Gudeg Nangka (Jogja)", natrium: 190, urt: "1 porsi" },
        soto_kadipiro: { nama: "Soto Bening (Kadipiro/Sleman)", natrium: 200, urt: "1 mangkuk" },
        oseng_mercon: { nama: "Oseng Mercon", natrium: 350, urt: "1 porsi" },
        mangut_lele: { nama: "Mangut Lele", natrium: 220, urt: "1 ekor" }
    },
    lauk_pauk: {
        ayam_goreng: { nama: "Ayam Goreng / kalasan", natrium: 173, urt: "1 potong" },
        ...
        sosis: { nama: "Sosis Sapi/Ayam", natrium: 400, urt: "1 buah" },
        nugget: { nama: "Chicken Nugget", natrium: 120, urt: "1 buah" },
        tempe_goreng: { nama: "Tempe Goreng", natrium: 50, urt: "1 potong" },
        tahu_goreng: { nama: "Tahu Goreng", natrium: 40, urt: "1 potong" },
        sarden_kaleng: { nama: "Sarden Kaleng", natrium: 310, urt: "1 kaleng" },
    ...
    camilan_saus: {
        ...
        geblek_sleman: { nama: "Geblek (Sleman)", natrium: 110, urt: "1 biji" },
...
natriumBtn?.addEventListener('click', hitungAKGv5);
...
// Data TKPI (Tabel Komposisi Pangan Indonesia) - Analisis Lemak per Porsi/Potong
const TKPI_DATABASE = {
    gorengan: {
        tempe: { nama: "Tempe Goreng", lemak: 10.4, urt: "1 potong" },
        tahu: { nama: "Tahu Goreng", lemak: 7.4, urt: "1 potong" },
        ...
        cireng: { nama: "Cireng Goreng", lemak: 1.6, urt: "1 biji" },
        mendoan: { nama: "Tempe Mendoan", lemak: 12.5, urt: "1 lembar" },
        tahu_susur: { nama: "Tahu Susur (Isi)", lemak: 8.2, urt: "1 biji" },
        geblek: { nama: "Geblek (Khas Kulon Progo/Sleman)", lemak: 1.2, urt: "1 biji" },
        jadah_goreng: { nama: "Jadah Goreng", lemak: 4.8, urt: "1 potong" }
    },
    lauk: {
        rendang: { nama: "Rendang Daging", lemak: 18.2, urt: "1 potong" },
        ayam_goreng: { nama: "Ayam Goreng (Kulit)", lemak: 15.4, urt: "1 potong" },
    ...
    masakan: {
        ...
        gudeg_nangka: { nama: "Gudeg Nangka (Santan)", lemak: 8.5, urt: "1 porsi" },
        krecek: { nama: "Sambal Goreng Krecek", lemak: 12.8, urt: "2 sdm" },
        soto_bening: { nama: "Soto Bening (Soto Kadipiro/Sleman)", lemak: 3.5, urt: "1 mangkuk" },
    ...
    bahan_tambahan: {
        ...
        bakpia_pathok: { nama: "Bakpia Pathok", lemak: 4.5, urt: "1 biji" },
...
function updateFoodCount(category, item, change) {
    ...
    const inputId = `count_${category}_${item}`;
    const input = document.getElementById(inputId);
    if (input) {
...
function resetFoodCalc() {
    ...
    Object.keys(TKPI_DATABASE).forEach(cat => {
        Object.keys(TKPI_DATABASE[cat]).forEach(item => {
            const inputId = `count_${cat}_${item}`;
            const input = document.getElementById(inputId);
            if (input) input.value = 0;
        });
    });
...
function toggleCategory(categoryId) {
    ...
    if (content && header) {
        ...
        Object.keys(TKPI_DATABASE).forEach(c => {
            const oc = document.getElementById(`cat_content_${c}`);
            const oh = document.getElementById(`cat_header_${c}`);
            if (oc && oh) {
...
function calculateTotalSelectedFat() {
    ...
    if (fatDisplay) animateNumber(fatDisplay, parseFloat(totalDisplay), 600, 1);
    if (fatDisplayLarge) animateNumber(fatDisplayLarge, parseFloat(totalDisplay), 600, 1);
                ...
                <div class="food-selector-container">
                        ${Object.keys(TKPI_DATABASE).map(cat => `
                            <div class="mb-2 food-category-card">
                                <div class="food-category-header collapsed" id="cat_header_${cat}" onclick="toggleCategory('${cat}')" role="button" aria-controls="cat_content_${cat}" aria-expanded="false" aria-label="Kategori ${cat.replace('_',' ')}">
                                ...
                                <div id="cat_content_${cat}" class="category-content d-none">
                                        ...
                                        return `
                                            <div class="food-item-row d-flex align-items-center justify-content-between mb-2 ps-2 gap-2" data-name="${food.nama}">
                                                ...
                                                <div class="input-group input-group-sm no-stack-mobile" style="width: 90px; flex-shrink: 0;">
                                                    ...
                                                    <input type="text" id="count_${cat}_${item}" class="form-control text-center py-1 px-1 fw-bold" value="0" readonly style="font-size: 0.85rem; background: #fff;">
                                                    <button class="btn btn-outline-warning py-1 px-2" onclick="updateFoodCount('${cat}', '${item}', 1)">+</button>
                                                </div>
                                            </div>
                                        `;
                    ...
                    <div class="lemak-legend-card mt-3">
                        <div class="d-flex align-items-center gap-2 mb-2">
                            ...
                            <strong class="small">Keterangan</strong>
                        </div>
                        <div class="lemak-legend-list">
                            <div class="legend-item legend-item-low"><span class="legend-dot legend-low"></span><span>Hijau = Kandungan lemak rendah</span></div>
                            <div class="legend-item legend-item-medium"><span class="legend-dot legend-medium"></span><span>Kuning = Kandungan lemak sedang</span></div>
                            <div class="legend-item legend-item-high"><span class="legend-dot legend-high"></span><span>Merah = Kandungan lemak tinggi</span></div>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="p-3">
                    ...
                    <div class="edu-fat-grid">
                        ...
                        <div class="edu-fat-card">
                            ...
                            <div>
                                <div class="edu-title">${TKPI_DATABASE.lauk.ayam_goreng.nama}</div>
                                <div class="edu-value">±${TKPI_DATABASE.lauk.ayam_goreng.lemak}g • ${TKPI_DATABASE.lauk.ayam_goreng.urt}</div>
                            </div>
                        </div>
                        <div class="edu-fat-card">
                        ...
                        <div class="edu-fat-card">
                            ...
                            <div>
                                <div class="edu-title">${TKPI_DATABASE.bahan_tambahan.santan_kental.nama}</div>
                                <div class="edu-value">±${TKPI_DATABASE.bahan_tambahan.santan_kental.lemak}g • ${TKPI_DATABASE.bahan_tambahan.santan_kental.urt}</div>
                            </div>
                        </div>
                        <div class="edu-fat-card">
                            <div class="edu-icon"><i class="fa-solid fa-bacon"></i></div>
                            <div>
                                <div class="edu-title">${TKPI_DATABASE.lauk.rendang.nama}</div>
                                <div class="edu-value">±${TKPI_DATABASE.lauk.rendang.lemak}g • ${TKPI_DATABASE.lauk.rendang.urt}</div>
                            </div>
                        </div>
                        <div class="edu-fat-card">
                            <div class="edu-icon"><i class="fa-solid fa-bowl-rice"></i></div>
